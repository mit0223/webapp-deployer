...
export function getConfig(): EnvironmentConfig {
  ...
  const config: EnvironmentConfig = {
    ...
    // IAMロール（必須）
    cdkDeployerRoleName: getEnvWithDefault(
      "CDK_DEPLOYER_ROLE_NAME",
      "CdkDeployer"
    ),

    // MFA設定（必須）
    mfaDeviceArn: getRequiredEnv("MFA_DEVICE_ARN"),
...
function validateConfig(config: EnvironmentConfig): void {
  ...
  if (!/^[a-z]{2}-[a-z]+-\d$/.test(config.awsRegion)) {
    throw new Error(
      "AWS_REGION の形式が正しくありません（例: ap-northeast-1）"
    );
  }
  ...
  if (!config.containerImage.includes("/")) {
    throw new Error(
      "CONTAINER_IMAGE の形式が正しくありません（例: ghcr.io/org/image:tag）"
    );
  }
